      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {eventConfig.tracks.map((track) => {
            const techPills: Record<string, string[]> = {
              ai: ["LLMs", "Neural Nets", "Vision", "Agents"],
              web: ["Next.js", "Full-Stack", "Web3", "UI/UX"],
              app: ["Flutter", "React Native", "Android", "iOS"],
              cloud: ["Serverless", "DevOps", "Docker", "Firebase"],
            };
            const pills = techPills[track.id] || ["Innovation", "Scalable", "Prototype"];
            ...
            return (
              <motion.div
                ...
                className="rounded-3xl border border-slate-200/90 bg-white p-6 shadow-sm hover:shadow-xl transition-all flex flex-col justify-between group relative overflow-hidden"
              >
                {/* Top Accent Gradient overlay */}
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${track.gradient} opacity-40 group-hover:opacity-100 transition-opacity pointer-events-none`}
                ></div>
                ...
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <span className="text-[10px] font-extrabold uppercase tracking-widest px-2.5 py-1 rounded-md bg-slate-100/90 text-slate-700 border border-slate-200">
                      {track.tag}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-6">
                    {pills.map((pill) => (
                      <span
                        key={pill}
                        className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-slate-200/60"
                      >
                        {pill}
                      </span>
                    ))}
                ...
                <div className="relative z-10 pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-semibold text-slate-500">
                  ...
                  <a
                    ...
                    className="text-blue-600 group-hover:translate-x-1 transition-transform font-bold inline-flex items-center gap-1"
                  >
                    <span>Explore</span>
                    <span>→</span>
                  </a>
